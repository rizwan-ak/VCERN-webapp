...
function Tickets({ currentUser, type, getPreSignedLink, uploadFile }) {
    ...
    useEffect(() => {
        join('vcern_admin', _id, organization);
        onJoin(
            val => setAllChats(val),
        ...
        onTicketEnded(val => {
            setCurrentChat(val);
            setFilteredChats(prevTickets => [...prevTickets, val]);
            setAllChats(prevTickets => [...prevTickets, val]);
        });
        onTicketCreated(
    ...
    console.log(`filteredChats`, allChats);
            ...
            <Grid item xs={12} md={3} className={!showList && classes.hideOnSm}>
                ...
                <div className={classes.chatListBox}>
                    <div className={classes.newChatBox}>
                        ...
                        <Add className={classes.newChatIcon} onClick={() => setShowNewTicketModal(true)} />
                    </div>
