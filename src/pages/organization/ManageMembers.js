...
function Settings({
  ...
  useEffect(() => {
    setFilteredMembers(poolMembers);
  }, [poolMembers]);
  useEffect(() => {
    console.log({ members });
    setFilteredMembers(
      poolMembers.filter(
        (el) =>
          ...
          el?.last_name?.toLowerCase().includes(searchAllMembers.toLowerCase())
      )
    );
  ...
  const handleRemove = () => {
    selectedMember.type === "Admin"
      ...
      : removeMemberFromPool(selectedMember.idx, token, () => {
          setSuccessMessage("Admin Removed Successfully");
          setPoolMembers(
            poolMembers.filter((el) => el.membership_id !== selectedMember.idx)
          );
          setShowConfirmationModal(false);
        });
  };
  ...
  const handleInviteMember = (idx) => {
    inviteToJoinPool({ pool: selectedPool?._id, members: [idx] }, token, () =>
  ...
  const handleInviteAllMembers = () => {
    inviteToJoinPool(
      { pool: selectedPool?._id, members: members.map((el) => el._id) },
      token,
      () => {
  ...
  return (
    <div>
      ...
      <div className={classes.inviteBox}>
        <div className={classes.inviteList}>
          ...
          <VCERNTypography
            ...
            variant="body1"
            value="Invite via text"
          />
          <div className={classes.inviteInput}>
            <InputMask
              mask="(+1) 999 999 9999"
              value={phone}
              onChange={(c) => setState({ ...state, phone: c.target.value })}
              maskChar=" "
            >
              {() => (
                <VCERNTextField
          ...
          <VCERNButton
            ...
            onClick={() => handleInviteTojoin("sms")}
          />
        </div>
        <div className={classes.inviteList}>
          ...
          <VCERNButton
            ...
            size="small"
            startIcon={icons.send}
            onClick={() => handleInviteTojoin("email")}
          />
        </div>
        <div className={classes.inviteList}>
          ...
          <div key={idx}>
            <div className={classes.listItem}>
              <VCERNAvatar
                ...
                src={el?.image}
                onClick={() =>
                  history.push(`/member/${el?._id}`, { member: el })
                }
              />
              <div className={classes.grow}>
                ...
                <VCERNTypography
                  className={classes.boldText}
                  variant="body2"
                  value={`Member Since ${getFormattedDate(el?.created)}`}
                  customColor="#6F7F9F"
                />
              </div>
              <VCERNButton
                ...
                value="Remove"
                size="small"
                onClick={() => handleActionClick(el?.membership_id, "Member")}
              />
            </div>
            <Divider />
          </div>
      ...
      {!isDefaultPool && (
        <VCERNButton
          value="Invite members to this pool"
          startIcon={icons.add}
            ...
            <div className={classes.listItem}>
              ...
              {el.type !== "super-admin" && (
                <VCERNButton
                  ...
                  value="Remove"
                  size="small"
                  onClick={() => handleActionClick(el?._id, "Admin")}
                />
              )}
            </div>
            <Divider />
            ...
            <>
              <div className={classes.listItem}>
                ...
                <VCERNButton
                  ...
                  value="Invite"
                  size="small"
                  onClick={() => handleInviteMember(el?._id)}
                />
              </div>
              <Divider />
            </>
