...
function Payments({
  ...
  useEffect(() => {
    ...
    fetchStats(
      selectedPool?._id,
      type === constants.USER_TYPE_VCERN ? constants.USER_TYPE_ORG : type,
      token,
      setStats
    );
  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid item xs={12}>
        {paymentHistory.map(
          ({ amount, payed_by_name, payed_by_image, date, type }, idx) => (
            <div key={idx}>
...
export default connect((state) => state, {
  ...
  fetchOrganizationsPayments: AC.fetchOrganizationsPayments,
