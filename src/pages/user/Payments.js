...
function Payments({
  ...
  const handleActionDelete = (idx) => {
    ...
    const card = methods[idx];
    if (card.id == defaultMethod) return;
    setSelectedCard(idx);
  ...
  return (
    <div>
        ...
        <Grid container spacing={3}>
          ...
          >
            <VCERNButton
              ...
              onClick={() => handleActionDelete(el?.id)}
            />
            {el?.id === defaultMethod ? (
              <VCERNTypography
                variant="body1"
                value="DEFAULT CARD"
                className={[classes.boldText, classes.defaultButton]}
              />
            ) : (
              <VCERNButton
                variant="outlined"
      ...
      <div className={classes.cardBox}>
        <Radio
          checked={selectedCard === 999}
          onChange={() => setSelectedCard(999)}
        />
        <VCERNTypography
          variant="body2"
          value="Add New Card"
      ...
      {selectedCard === 999 && (
        <div>
          ...
          />
          <VCERNTypography
            value={`Once a year, you will be charged a $${userSubscriptionPrice}  membership fee by vCERN.`}
            variant="body2"
          />
          <VCERNTypography variant="body2">
            ...
            />
            will collect a contribution from you every time someone passes away
            in this pool. This contribution is determined by dividng the payout
            amount by the number of people in this pool.
          </VCERNTypography>
          <VCERNTypography
            value=" Your beneficiary will collect $20,000 in the event of your passing."
            variant="body1"
            className={classes.boldText}
          />
          <CardElement
          ...
          <div className={classes.remberMeContainer}>
            <Checkbox
              ...
              value={agreedTerms}
              checked={agreedTerms}
              onChange={(evt) => setAgreedTerms(evt.target.checked)}
            />
      ...
      {paymentHistory.map(
        ({ amount, payed_for_name, payed_for_image, date, type }, idx) => (
          <div key={idx}>
            <div className={classes.listBox}>
              ...
              <div className={classes.grow}>
                ...
                <VCERNTypography
                  variant="caption"
                  value={
                    type === "EVENT_PAYMENT" ? "Contribution" : "Membership Fee"
                  }
