...
const useStyles = makeStyles((theme) => ({
  ...
  waitingPic: { height: 300, maxWidth: "98%", objectFit: "contain" },
  ...
  activeBox: {
    ...
    margin: "0 auto",
    textAlign: "center",
    [theme.breakpoints.down("sm")]: { padding: "10px 5px" },
  },
...
function Dashboard({
  ...
  const daysCompleted = moment().diff(
    moment(new Date(currentUser.created)),
    "days"
  );
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        ...
        <VCERNTypography
          ...
          value={`Welcome to the ${selectedPool?.name}!`}
        />
      </Grid>
      <Grid item xs={12}>
        {status === "active" ? (
              ...
              <Grid item xs={12} md={8} lg={8} className={classes.innerBox}>
                ...
                <div>
                  <VCERNTypography
                    ...
                    variant="h4"
                    align="center"
                    value="Hey! You’re in the waiting pool now!"
                  />
                  <VCERNTypography
                    className={classes.text}
                    variant="h5"
                  ...
                  <VCERNTypography
                    ...
                    variant="h6"
                    align="center"
                    value={`${90 - daysCompleted} days remaining to unlock!`}
                  />
                  <LinearProgress
                    variant="determinate"
                    color="secondary"
                    value={100 * (daysCompleted / 90)}
                  />
                  <VCERNTypography
                    className={classes.text}
                    variant="body2"
        ...
        {status === "active" ? (
          <Paper className={classes.paper}>
            <VCERNTypography
              ...
              value={`Total Members: ${stats?.number_of_members}`}
            />
            <VCERNPieChart
              data={[
                {
                  name: "Pending",
                  value: stats?.number_of_pending,
                  color: "#FE9900",
                },
                {
                  name: "Deceased",
                  value: stats?.number_of_deceased,
                  color: "#C90000",
                },
                {
                  name: "Active",
                  value: stats?.number_of_active,
                  color: "#1DD1A1",
                },
                {
                  name: "Delinquent",
                  value: stats?.number_of_deliquent,
                  color: "#07A7E3",
                },
              ]}
            />
          ...
          <Paper className={classes.inviteBox}>
            ...
            <VCERNTypography
              variant="body1"
              value="Upload requirement documents to"
            />
            <VCERNTypography
              variant="body1"
              value="activate you membership after the 3"
            />
          ...
          <div className={classes.contributeBox}>
            ...
            <div>
              <VCERNTypography
                variant="body1"
                className={classes.inviteText}
                value={`Contribution: $${stats?.contribution}`}
              />
              <VCERNTypography
                variant="body2"
                className={classes.inviteText}
                value="I’ll donate each time"
                customColor="#6F7F9F"
              />
            </div>
          </div>
      ...
      <Grid item xs={12} md={6}>
        <Paper className={classes.inviteBox}>
          ...
          <VCERNTypography
            className={classes.inviteText}
            variant="body1"
            value="Help your pool grow faster by inviting family and friends!"
          />
          <VCERNTypography
            variant="body2"
            value="Get $5  everytime someone you referred joins this pool"
            customColor="#6F7F9F"
          />
          <VCERNButton
            fullWidth
            value="Invite"
            startIcon={icons.invite}
            className={classes.continueButton}
          />
        </Paper>
      </Grid>
      ...
      {!!events.length && status === "active" && (
        <>
          ...
          <Grid
            container
            spacing={3}
            style={{ margin: "0 0 20px 0", justifyContent: "center" }}
          >
            <VCERNButton
              fullWidth
              startIcon={icons.eye}
              value={"View All Deaths"}
              onClick={() => history.push("/events", events)}
            />
          </Grid>
        </>
      )}
